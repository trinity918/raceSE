...
const CoverLetter = () => {
  ...
  const apiUrl = 'http://127.0.0.1:5000'; // Your flask server URL
  ...
  const findCoursesForSkills = (missingSkills) => {
    if (!missingSkills || missingSkills.length === 0 || !Array.isArray(coursesData)) {
      return [];
    }
    ...
    missingSkills.forEach(skill => {
      ...
      const matchingCourses = coursesData.filter(course => {
        // Check if courseName contains the skill
        const courseNameMatch = course.courseName && 
                              course.courseName.toLowerCase().includes(skillLower);
        
        // If course has skills array, check that too (in case you add it later)
        const skillsMatch = course.skills && 
                            Array.isArray(course.skills) && 
                            course.skills.some(s => s.toLowerCase().includes(skillLower));
        
        return courseNameMatch || skillsMatch;
      });
      
      courses.push(...matchingCourses);
    });
  
    // Remove duplicates using courseLink as unique identifier
    return [...new Map(courses.map(item => [item.courseLink, item])).values()];
  };
              ...
              {recommendedCourses.length > 0 ? (
                  ...
                  >
                    <div className="p-6">
                      ...
                      <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
                        by {course.courseTeacher} • {course.courseProvider}
                      </p>
                ...
                <div className="col-span-full text-center py-12">
                  <p className="text-gray-600 dark:text-gray-400">
                    No courses recommended yet. Generate a cover letter first to get skill recommendations.
                  </p>
                </div>
              )}
            </div>
